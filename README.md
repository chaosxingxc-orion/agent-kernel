...
> **事件流配置**：`TemporalSDKWorkflowGateway` 在 `TemporalGatewayConfig.event_stream_query_method_name` 为 `None`（默认值）时调用 `stream_run_events` 会抛出 `RuntimeError`。如需使用 SSE 事件流，须显式设置此字段：
>
> ```python
> from agent_kernel.substrate.temporal.gateway import TemporalGatewayConfig, TemporalSDKWorkflowGateway
>
> gateway = TemporalSDKWorkflowGateway(
>     temporal_client,
>     config=TemporalGatewayConfig(
>         event_stream_query_method_name="query_runtime_events",  # 必填，与工作流查询方法名一致
>     ),
> )
> ```
...
| GET | `/runs/{run_id}/events` | SSE 流式订阅事件 (`stream_run_events`)；Temporal 模式下须在 `TemporalGatewayConfig` 中设置 `event_stream_query_method_name`，否则抛出 `RuntimeError` |
