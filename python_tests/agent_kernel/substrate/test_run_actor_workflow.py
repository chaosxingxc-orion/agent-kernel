...
def test_signal_requires_run_context_before_processing() -> None:
    """Workflow should reject signals before run() establishes identity."""
    ...
    with pytest.raises(
        RuntimeError,
        match=r"RunActorWorkflow\.run must be called first",
    ):
        asyncio.run(
            workflow.signal(
                ActorSignal(signal_type="external_callback")
            )
        )
