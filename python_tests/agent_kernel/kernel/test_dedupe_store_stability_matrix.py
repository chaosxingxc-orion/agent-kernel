...
def test_dedupe_matrix_reserve_and_terminal_state(seed: int) -> None:
    ...
    if seed % 2 == 0:
        ...
        assert store.get(envelope.dispatch_idempotency_key).state == "acknowledged"  # type: ignore[union-attr]
    else:
        store.mark_unknown_effect(envelope.dispatch_idempotency_key)
        assert store.get(envelope.dispatch_idempotency_key).state == "unknown_effect"  # type: ignore[union-attr]
